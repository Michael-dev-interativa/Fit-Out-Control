...
async function resolveEmpreendimentoId(pool, record, legacyMap, cache) {
  ...
  if (empreendimentoNome) {
    ...
    try {
      ...
      console.warn(`  ⚠️  Query failed for "${empreendimentoNome}": ${err.message}`);
    }
  }
  ...
  if (legacyMap.has(legacyId)) {
    ...
    try {
      ...
      console.warn(`  ⚠️  Query failed for legacy "${nome}": ${err.message}`);
    }
  }
...
async function findExisting(pool, payload) {
  try {
    ...
    console.warn(`  ⚠️  findExisting failed: ${err.message}`);
    return null;
  }
}
