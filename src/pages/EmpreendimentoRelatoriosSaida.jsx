...
import { UnidadeEmpreendimento } from '@/entities/UnidadeEmpreendimento';
...
const translations = {
  pt: {
    title: "Relatórios de Saída",
    backToProject: "Voltar ao Empreendimento",
    newReport: "Novo Relatório de Saída",
    noReports: "Nenhum relatório de saída encontrado",
    addFirstReport: "Adicione o primeiro relatório de saída",
    unit: "Unidade",
    tenant: "Locatário",
    date: "Data",
    status: "Status",
    ...
    selectUnitDescription: "Escolha a unidade para criar o relatório de saída",
  ...
  en: {
    ...
    newReport: "New Exit Report",
    noReports: "No exit reports found",
    addFirstReport: "Add the first exit report",
    unit: "Unit",
    tenant: "Tenant",
    date: "Date",
    status: "Status",
    ...
    selectUnitDescription: "Choose the unit to create the exit report",
...
const SelectUnitDialog = ({ open, onOpenChange, empreendimentoId, language, theme }) => {
  ...
  const handleSelectUnidade = (unidade) => {
    navigate(createPageUrl(`PreencherRelatorioSaida?unidadeId=${unidade.id}&empreendimentoId=${empreendimentoId}`));
    onOpenChange(false);
  };
...
export default function EmpreendimentoRelatoriosSaida({ language: initialLanguage, theme: initialTheme }) {
  const navigate = useNavigate();
  ...
  const loadData = async () => {
    ...
    try {
      const [empData, relatoriosData, unidadesData] = await Promise.all([
        ...
        RelatorioSaida.filter({ id_empreendimento: empreendimentoId }, '-created_date'),
  ...
  const handleDelete = async (relatorioId) => {
    ...
    try {
      await RelatorioSaida.delete(relatorioId);
      await loadData();
    } catch (error) {
  ...
  const handleView = (relatorio) => {
    navigate(createPageUrl(`VisualizarRelatorioSaida?relatorioId=${relatorio.id}`));
  };

  const handleEdit = (relatorio) => {
    navigate(createPageUrl(`PreencherRelatorioSaida?relatorioId=${relatorio.id}&unidadeId=${relatorio.id_unidade}&empreendimentoId=${empreendimentoId}`));
  };
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className={`text-sm mt-2 space-y-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        ...
                        <p><strong>{t.date}:</strong> {relatorio.data_saida ? format(parseLocalDate(relatorio.data_saida), "dd/MM/yyyy", { locale: pt }) : '-'}</p>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      <Badge className={getStatusBadgeClass(relatorio.status_saida)}>
                        {relatorio.status_saida}
                      </Badge>
                      <div className="flex gap-2">
                        <Button 
                          variant="outline" 
