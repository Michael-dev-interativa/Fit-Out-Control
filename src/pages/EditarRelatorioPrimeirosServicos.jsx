...
const t = {
    title: "Editar Relatório de 1º Serviços",
...
export default function EditarRelatorioPrimeirosServicos() {
    const navigate = useNavigate();
    const location = useLocation();
    const [relatorioId] = useState(() => new URLSearchParams(location.search).get('relatorioId'));
    ...
    const [activeAprovacaoIndex, setActiveAprovacaoIndex] = useState(null);
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                const data = await RelatorioPrimeirosServicos.get(relatorioId);
                setFormData({
                    ...data,
                    data_relatorio: data.data_relatorio ? data.data_relatorio.split('T')[0] : '',
                    fotos: data.fotos || [],
    ...
    const handleSaveSignature = async () => {
        if (activeAprovacaoIndex === null) return;
        ...
        try {
            if (signatureMode === 'type') {
                ...
                const { file_url } = await UploadFile({ file });
                handleAprovacaoChange(activeAprovacaoIndex, 'assinatura_imagem', file_url);
                setShowSignatureDialog(false);
                setActiveAprovacaoIndex(null);
                setTypedSignature('');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await RelatorioPrimeirosServicos.update(id, updateData);
            toast.success("Registro atualizado com sucesso!");
            navigate(createPageUrl(`EmpreendimentoPrimeirosServicos?empreendimentoId=${formData.id_empreendimento}`));
        } catch (error) {
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Card>
                    ...
                    <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><Label>{t.status}</Label><Select value={formData.status} onValueChange={v => handleInputChange('status', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{statusOptions.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
