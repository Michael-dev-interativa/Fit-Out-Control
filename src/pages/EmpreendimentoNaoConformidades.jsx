...
import { Loader2, Plus, ArrowLeft, Trash2, Pencil, Eye, Copy } from 'lucide-react';
...
const t = {
    ...
    loading: "Carregando não conformidades...",
    add: "Nova Não Conformidade",
    noRecords: "Nenhuma não conformidade encontrada.",
    back: "Voltar ao Empreendimento",
    delete: "Excluir",
    edit: "Editar",
    view: "Visualizar",
    confirmDeleteTitle: "Confirmar Exclusão",
    confirmDeleteMsg: "Tem certeza de que deseja excluir esta não conformidade? Esta ação não pode ser desfeita.",
    cancel: "Cancelar",
    date: "Data",
    client: "Cliente",
    revision: "Revisão",
...
export default function EmpreendimentoNaoConformidades({ language: initialLanguage, theme: initialTheme }) {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const naoConformidadesData = await NaoConformidade.filter({ id_empreendimento: empreendimentoId }, "-data_vistoria");
            setEmpreendimento(empreendimentoData);
            setNaoConformidades(naoConformidadesData);
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        try {
            await NaoConformidade.delete(id);
            loadData();
        } catch (error) {
    ...
    const handleCopy = async (id) => {
        try {
            const originalData = await NaoConformidade.get(id);
            const { id: _, created_date, updated_date, created_by, ...dadosCopia } = originalData;
            dadosCopia.titulo_relatorio = `${dadosCopia.titulo_relatorio} - Cópia`;
            await NaoConformidade.create(dadosCopia);
            loadData();
        } catch (error) {
        ...
        <div className={`p-6 ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-50'}`}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={() => navigate(createPageUrl(`NovaNaoConformidade?empreendimentoId=${empreendimentoId}`))}>
            ...
            {loadingData ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {naoConformidades.map((item) => (
                        <Card key={item.id} className={isDark ? 'bg-gray-800' : ''}>
                            <CardHeader>
                            ...
                            <CardContent>
                                ...
                                <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{t.revision}: {item.revisao || '-'}</p>
                            </CardContent>
                            <CardFooter className="flex justify-end gap-2">
                                <div className="flex items-center gap-2">
                                    <Link to={createPageUrl(`VisualizarNaoConformidade?relatorioId=${item.id}`)}>
                                        <Button variant="ghost" size="icon"><Eye className="w-4 h-4" /></Button>
                                    </Link>
                                    <Link to={createPageUrl(`EditarNaoConformidade?id=${item.id}`)}>
                                        <Button variant="ghost" size="icon"><Pencil className="w-4 h-4" /></Button>
                                    </Link>
                                    <Button variant="ghost" size="icon" onClick={() => handleCopy(item.id)}>
