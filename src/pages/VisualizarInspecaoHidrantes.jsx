...
const FotoInspecao = ({ url, legenda, maxHeight = '250px' }) => {
    ...
    return (
        <div className="text-center foto-inspecao">
            <img className="foto-inspecao-img" src={url} data-original={url} height={maxHeight} alt={legenda || 'Foto da inspeção'} style={{ width: '100%', height: maxHeight, minHeight: '200px', maxHeight: '260px', objectFit: 'cover', border: '1px solid #ddd', display: 'block' }} />
            {legenda && (
                <p className="text-[9px] text-gray-600 mt-1">{legenda}</p>
            )}
        </div>
        ...
        <div className={showHeader ? "p-4" : "px-4 pt-6 pb-4"}>
            ...
            {itensSlice && itensSlice.length > 0 && (
                <div>
                    {showHeader && (
                        <>
                            <h3 className="text-lg font-bold mb-2 bg-blue-900 text-white p-2 text-center">Inspeção Física - Hidrantes</h3>
            ...
            {!isCover && (
                <div className="flex justify-between items-center border-b border-gray-200 bg-white" style={{ position: 'absolute', top: 0, left: 0, right: 0, height: HEADER_HEIGHT, zIndex: 100, padding: '4px 8px', maxWidth: '210mm', boxSizing: 'border-box' }}>
                    ...
                    <div className="text-right" style={{ flex: 1, paddingLeft: '8px', overflow: 'hidden' }}>
                        <h2 className="text-[10px] font-bold text-gray-800 uppercase leading-tight truncate">{relatorio?.titulo_relatorio || 'INSPEÇÃO DE HIDRANTES'}</h2>
...
const ReportContent = ({ relatorio, empreendimento, navigate }) => {
    ...
    const totalPages = 1 + (hasDocumentacao ? 1 : 0) + contentPages.length + 2 + (hasAssinaturas ? 1 : 0);
    let currentPage = 1;
        ...
        <div className="bg-gray-200 print:bg-white min-h-screen font-sans">
            <div className="no-print shadow-sm border-b p-4 mb-4 bg-white">
                <div className="flex justify-between items-center max-w-4xl mx-auto">
                    ...
                    <h1 className="text-xl font-semibold text-gray-800">Visualizar Inspeção de Hidrantes</h1>
...
export default function VisualizarInspecaoHidrantes() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const relatorioData = await InspecaoHidrantes.get(relatorioId);
