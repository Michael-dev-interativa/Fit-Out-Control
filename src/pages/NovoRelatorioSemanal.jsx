...
const translations = {
  pt: {
    ...
    uploading: "Enviando...",
    approvals: "Vistos", // This translation key is now unused but kept for consistency
    addApproval: "Adicionar Visto", // This translation key is now unused but kept for consistency
    responsible: "Responsável (Ex: Ger. Obra)", // This translation key is now unused but kept for consistency
    approverName: "Nome do Aprovador", // This translation key is now unused but kept for consistency
...
export default function NovoRelatorioSemanal() {
    ...
    // Memoize handleInputChange to ensure its stability across renders
    const handleInputChange = useCallback((field, value) => {
        setFormData(p => ({ ...p, [field]: value }));
    }, []); // No dependencies, so it's stable

    // Effect to calculate fisico_previsto_total and fisico_real_total
    useEffect(() => {
        if (formData.avanco_fisico_acumulado && formData.avanco_fisico_acumulado.length > 0) {
            const sortedWeeks = [...formData.avanco_fisico_acumulado]
                .filter(item => item.semana)
                .sort((a, b) => new Date(b.semana) - new Date(a.semana));
            
            if (sortedWeeks.length > 0) {
                const latestWeek = sortedWeeks[0];
                handleInputChange('fisico_previsto_total', latestWeek.planejado || '');
                handleInputChange('fisico_real_total', latestWeek.realizado || '');
            } else {
    ...
    }, [formData.avanco_fisico_acumulado, handleInputChange]);

    const formatDate = (dateString) => {
        if (!dateString) return '-';
        // Treat the date as UTC by appending 'T00:00:00' to avoid timezone shifts
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await RelatorioSemanal.create(finalData);
            ...
            toast.error("Falha ao criar o relatório semanal.");
        } finally {
            setSaving(false);
        }
    };
                            ...
                            <DialogHeader>
                                ...
                                <DialogDescription as="div" className="prose prose-sm max-w-none pt-4 text-gray-600">
                                    ...
                                    <ul className="list-disc pl-5 space-y-1">
                                        ...
                                        <li><b>Concluído:</b> Preencher o avanço físico da obra (%).</li>
                                ...
                                {formData.principais_atividades_semana.map((item, index) => (
                                    <div key={index} className="p-4 border rounded-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative">
                                        ...
                                        <div className="col-span-1"><Label>Status</Label>
                                            <Select value={item.status} onValueChange={(value) => handleTableChange('principais_atividades_semana', index, 'status', value)}>
                            ...
                            <div className="space-y-4">
                                {formData.atividades_proxima_semana_tabela.map((item, index) => (
                                     <div key={index} className="p-4 border rounded-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative">
                                        ...
                                        <div className="col-span-1"><Label>Status</Label>
                                            <Select value={item.status} onValueChange={(value) => handleTableChange('atividades_proxima_semana_tabela', index, 'status', value)}>
                                                <SelectTrigger><SelectValue /></SelectTrigger>
                                                <SelectContent>
                                                    <SelectItem value="Não Iniciado">Não Iniciado</SelectItem>
                                                    <SelectItem value="Em Andamento">Em Andamento</SelectItem>
