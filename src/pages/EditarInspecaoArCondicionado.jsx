...
const t = {
    title: "Editar Inspeção de Ar Condicionado",
...
export default function EditarInspecaoArCondicionado() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                const data = await InspecaoArCondicionado.get(inspecaoId);
                let dataInspecao = '';
                ...
                let dataProj = '';
                if (data.data_projeto) {
                    dataProj = data.data_projeto.includes('T') 
                        ? data.data_projeto.split('T')[0] 
                        : data.data_projeto;
                }
                ...
                setFormData({
                    ...
                    data_projeto: dataProj,
                    projeto: data.projeto || '',
                    titulo_secao_inspecao: data.titulo_secao_inspecao || 'Inspeção Física - Sistema de Ar Condicionado',
                    evaporadoras: data.evaporadoras || [],
                    condensadoras: data.condensadoras || [],
                    ...
                    inspecao_evaporadora: data.inspecao_evaporadora || { itens: [], comentarios: '' },
                    inspecao_valvulas: data.inspecao_valvulas || { itens: [], comentarios: '' },
                    inspecao_condensadora: data.inspecao_condensadora || { itens: [], comentarios: '' },
                    inspecao_eletrica: data.inspecao_eletrica || { itens: [], comentarios: '' },
                    inspecao_sensores: data.inspecao_sensores || { itens: [], comentarios: '' },
                });
            } catch (error) {
    ...
    const addInspecaoSectionItem = (sectionKey) => {
        const newSection = { ...formData[sectionKey] };
        if (!newSection.itens) newSection.itens = [];
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await InspecaoArCondicionado.update(id, updateData);
            toast.success("Inspeção atualizada com sucesso!");
            navigate(createPageUrl(`EmpreendimentoInspecaoArCondicionado?empreendimentoId=${formData.id_empreendimento}`));
        } catch (error) {
    ...
    return (
        <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {[
                    { key: 'inspecao_evaporadora', titulo: 'INSPEÇÃO FÍSICA - UNIDADE EVAPORADORA' },
                    { key: 'inspecao_valvulas', titulo: 'INSPEÇÃO FÍSICA - VÁLVULAS E TUBULAÇÃO FRIGORÍGENA' },
                    { key: 'inspecao_condensadora', titulo: 'INSPEÇÃO FÍSICA - UNIDADE CONDENSADORA' },
                    { key: 'inspecao_eletrica', titulo: 'INSPEÇÃO FÍSICA - ELÉTRICA E CONTROLES' },
                    { key: 'inspecao_sensores', titulo: 'INSPEÇÃO FÍSICA - SENSORES E CONTROLES' },
                ].map(section => (
