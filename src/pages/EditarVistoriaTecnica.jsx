...
export default function EditarVistoriaTecnica() {
        ...
        const load = async () => {
            try {
                ...
                setCoverData({
                    ...
                    capa_area_top: data.capa_area_top || '50%',
                    capa_area_right: data.capa_area_right || '-3%',
                    capa_area_left: data.capa_area_left || '',
                    capa_area_width: data.capa_area_width || '45%',
                    capa_area_align: data.capa_area_align || 'center',
                    capa_area_bg: data.capa_area_bg || 'transparent',
                    capa_area_padding: data.capa_area_padding || '0',
                    capa_area_radius: data.capa_area_radius || '0',
                    capa_area_titulo_font_size: data.capa_area_titulo_font_size || '26px',
                    capa_area_titulo_color: data.capa_area_titulo_color || '#000000',
                    capa_area_titulo_margin_bottom: data.capa_area_titulo_margin_bottom || '6px',
                    capa_area_subtitulo_font_size: data.capa_area_subtitulo_font_size || '16px',
    ...
    const handleItemFoto = async (localIdx, itemIdx, file) => {
        if (!file) return;
        try {
            const result = await uploadPhoto(file);
            const locais = [...(formData.locais || [])];
            const itens = [...(locais[localIdx].itens_inspecao || [])];
            const fotos = [...(itens[itemIdx].fotos || []), { url: result.url, legenda: '' }];
            itens[itemIdx] = { ...itens[itemIdx], fotos };
            locais[localIdx] = { ...locais[localIdx], itens_inspecao: itens };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error(error);
            toast.error("Falha ao atualizar a vistoria técnica.");
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold">Editar Vistoria Técnica</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Card>
                    ...
                    <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input type="date" value={formData.data_vistoria ? formData.data_vistoria.split('T')[0] : ''} onChange={e => handleChange('data_vistoria', e.target.value)} />
                        </div>
                        <div className="space-y-2">
                ...
                <Card>
                    <CardHeader><CardTitle>Tópico 1 — Características Gerais</CardTitle></CardHeader>
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            {formData.foto_localizacao ? (
                                <div className="relative inline-block">
                                    <img src={formData.foto_localizacao} alt="Localização" className="rounded-lg border max-h-80 object-contain" />
                                    <button type="button" onClick={() => handleChange('foto_localizacao', '')} className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600">
                                        <X className="w-4 h-4" />
                                    </button>
                                </div>
                            ) : (
                        ...
                        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center gap-4"><span className="font-bold text-green-700 w-8 text-center">OK</span><span className="text-sm text-gray-700">Itens em conformidade</span></div>
                                <div className="flex items-center gap-4"><span className="font-bold text-yellow-700 w-8 text-center">PD</span><span className="text-sm text-gray-700">Itens pendentes a serem revisados</span></div>
                                <div className="flex items-center gap-4"><span className="font-bold text-blue-700 w-8 text-center">SG</span><span className="text-sm text-gray-700">Itens com sugestão de melhorias</span></div>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div>
                            <h3 className="text-sm font-semibold text-gray-700 mb-2">Lista Mestra de Documentos Analisados</h3>
                        <div>
                            <table className="w-full text-sm border-collapse" style={{tableLayout:'fixed'}}>
                                <thead>
                                    <tr className="bg-blue-900 text-white">
                                        <th className="border border-blue-700 p-2 text-left" style={{width:'8%'}}>DES</th>
                                        <th className="border border-blue-700 p-2 text-left" style={{width:'30%'}}>Descrição</th>
                                        <th className="border border-blue-700 p-2 text-left" style={{width:'30%'}}>Arquivo</th>
                                        <th className="border border-blue-700 p-2 text-left" style={{width:'10%'}}>Rev</th>
                                        <th className="border border-blue-700 p-2 text-left" style={{width:'15%'}}>Data</th>
                        ...
                        <div>
                            <h3 className="text-sm font-semibold text-gray-700 mb-2">Normas Técnicas e Códigos</h3>
                ...
                <Card>
                    <CardHeader>
                        <div className="flex justify-between items-center">
                            ...
                            <Button type="button" size="sm" onClick={() => handleChange('locais', [...(formData.locais||[]), { nome_local: '', fotos: [], descricao_geral: '', comentarios: '', itens_inspecao: [] }])}>
                            ...
                            <div key={lIdx} className="border border-gray-200 rounded-lg overflow-hidden">
                                ...
                                <div className="bg-blue-900 text-white px-4 py-2 flex items-center justify-between">
                                    <Input
                                        ...
                                        onChange={e => {
                                            ...
                                            l[lIdx] = { ...l[lIdx], nome_local: e.target.value };
                                            handleChange('locais', l);
                                        }}
                                        placeholder="Nome do Tópico (ex: Ambientes, Cobertura, Casa de Máquinas...)"
                                ...
                                <div className="p-4 space-y-4">
                                        ...
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {(local.fotos || []).map((foto, fIdx) => (
                                                <div key={fIdx} className="relative">
                                                    ...
                                                    <button type="button" onClick={() => {
                                                        const l = [...(formData.locais||[])];
                                                        const fs = [...(l[lIdx].fotos||[])];
                                                        fs.splice(fIdx, 1);
                                                        l[lIdx] = { ...l[lIdx], fotos: fs };
                                                        handleChange('locais', l);
                                            ...
                                            <label className="w-32 h-32 border-2 border-dashed border-gray-300 rounded flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50">
                                                ...
                                                <input type="file" accept="image/*" multiple className="hidden" onChange={async e => {
                                                    ...
                                                    try {
                                                        ...
                                                        setFormData(prev => {
                                                            const l = [...(prev.locais||[])];
                                                            l[lIdx] = { ...l[lIdx], fotos: [...(l[lIdx].fotos||[]), ...novasfotos] };
                                                            return { ...prev, locais: l };
                                                        });
                                                    } catch { toast.error('Erro ao fazer upload.'); }
                                                }} />
                                            </label>
                                    ...
                                    <div>
                                        ...
                                        <Textarea
                                            value={local.descricao_geral || ''}
                                            onChange={e => {
                                                const l = [...(formData.locais||[])];
                                                l[lIdx] = { ...l[lIdx], descricao_geral: e.target.value };
                                                handleChange('locais', l);
                                            }}
                                            rows={3}
                                            className="mt-1"
                                            placeholder="Descrição geral do local..."
                                        />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <Textarea
                                            ...
                                            onChange={e => {
                                                const l = [...(formData.locais||[])];
                                                l[lIdx] = { ...l[lIdx], comentarios: e.target.value };
                                                handleChange('locais', l);
                                            }}
                                            rows={3}
                                            className="mt-1"
                                            placeholder="Comentários gerais sobre o local..."
                                        />
                                    </div>
                                    ...
                                    <div>
                                                        ...
                                                        <tr key={iIdx} className="hover:bg-gray-50">
                                                            <td className="border border-gray-300 p-1">
                                                                <Input value={item.descricao || ''} onChange={e => {
                                                                    ...
                                                                    its[iIdx] = { ...its[iIdx], descricao: e.target.value };
                                                            ...
                                                            <td className="border border-gray-300 p-1 text-center">
                                                                <select value={item.resultado || 'OK'} onChange={e => {
                                                                    const l = [...(formData.locais||[])];
                                                                    const its = [...(l[lIdx].itens_inspecao||[])];
                                                                    its[iIdx] = { ...its[iIdx], resultado: e.target.value };
                                                                    l[lIdx] = { ...l[lIdx], itens_inspecao: its };
                                                                    handleChange('locais', l);
                                                                }} className={`w-full h-7 text-xs font-bold rounded px-1 border-0 ${ {'OK':'bg-green-100 text-green-700','PD':'bg-red-100 text-red-700','SG':'bg-blue-100 text-blue-700','N/OK':'bg-orange-100 text-orange-700'}[item.resultado||'OK'] || '' }`}>
                                        ...
                                        <div className="flex gap-2 mt-2">
                                            <Button type="button" variant="outline" size="sm" onClick={() => {
                                                const l = [...(formData.locais||[])];
                                                const its = [...(l[lIdx].itens_inspecao||[]), { descricao: '', resultado: 'OK', observacoes: '' }];
                                                l[lIdx] = { ...l[lIdx], itens_inspecao: its };
                                                handleChange('locais', l);
                                            }}>
                                    ...
                                    {/* Botão para adicionar novo local dentro do mesmo tópico */}
                                    <div className="px-4 pb-4">
                                    <Button type="button" variant="outline" size="sm" className="w-full border-dashed border-blue-400 text-blue-700 hover:bg-blue-50" onClick={() => {
                                        const l = [...(formData.locais||[])];
                                        const novoLocal = { nome_local: local.nome_local, nome_local_exibicao: '', fotos: [], descricao_geral: '', comentarios: '', itens_inspecao: [], is_sub_local: true };
                                        l.splice(lIdx + 1, 0, novoLocal);
                                        handleChange('locais', l);
                                    }}>
                                        <Plus className="w-4 h-4 mr-1" /> Adicionar Local neste Tópico
                                    </Button>
                                    </div>
                                    </div>
                                    ))}
                ...
                <Card>
                    <CardHeader>
                        <div className="flex justify-between items-center">
                            ...
                            <Button type="button" size="sm" onClick={() => handleChange('quadros_gerais', [...(formData.quadros_gerais||[]), { nome_topico: '', nome_local: '', nomenclatura: '', fotos: [], itens: [
                            ...
                            <div key={qIdx} className="border border-gray-200 rounded-lg overflow-hidden">
                                ...
                                <div className="p-4 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <Textarea
                                            value={quadro.itens_analisados || ''}
                                            onChange={e => { const q = [...(formData.quadros_gerais||[])]; q[qIdx] = {...q[qIdx], itens_analisados: e.target.value}; handleChange('quadros_gerais', q); }}
                                            rows={4}
                                            className="mt-1"
                                            placeholder="Descreva os itens analisados neste quadro..."
                                        />
                                        </div>
                                        <div>
                                        ...
                                        <div className="space-y-2">
                                            {(quadro.notas || []).map((nota, nIdx) => (
                                                <div key={nIdx} className="flex items-center gap-2">
                                                    ...
                                                    <Input
                                                        value={nota}
                                                        onChange={e => { const q = [...(formData.quadros_gerais||[])]; const ns = [...(q[qIdx].notas||[])]; ns[nIdx] = e.target.value; q[qIdx] = {...q[qIdx], notas: ns}; handleChange('quadros_gerais', q); }}
                                                        className="h-7 text-xs flex-1"
                                                        placeholder={`Nota ${nIdx + 1}...`}
                                                    />
