...
const translations = {
  pt: {
    ...
    formNotFound: (formId) => `O modelo de formulário (ID: ${formId}) associado a este termo não foi encontrado ou está vazio. Não é possível gerar o relatório.`,
    notApplicable: "N/A",
  },
  en: {
    title: "Acceptance Term",
    ...
    formNotFound: (formId) => `The form template (ID: ${formId}) associated with this term was not found or is empty. The report cannot be generated.`,
    notApplicable: "N/A",
  }
};

const ReportPage = ({ children, pageNumber, totalPages, termo, unidade, empreendimento, pdfMode }) => {
        ...
        >
          ...
          <div className="text-right">
            <h2 className="text-sm font-bold text-gray-800 uppercase">
              {termo?.nome_termo || "TERMO DE ACEITE"}
            </h2>
      ...
      {pageNumber > 1 && (
        ...
        >
          <div className="flex-1 text-left">
            ...
            <br />
            <span>{termo?.nome_arquivo || 'N/A'}</span>
          </div>
...
const CoverPage = ({ termo, unidade, empreendimento, items, t, pdfMode, formulario }) => {
  const year = new Date(termo?.data_termo || Date.now()).getFullYear();
    ...
    <div className="report-page relative w-full h-full bg-white font-sans overflow-hidden" style={{ margin: 0, padding: 5 }}>
      ...
      <div className="absolute z-30" style={{ top: '10%', right: '8%', width: '50%', textAlign: 'right' }}>
        ...
        <h2 style={{ fontFamily: "'Inter', sans-serif", fontSize: '29.5px', color: redColor, letterSpacing: '4px' }}>
          {formulario?.nome_tipo_vistoria || 'TERMO DE ACEITE'}
        </h2>
      </div>
...
const QRCodesPage = ({ allItems, termo, pdfMode }) => {
  const allPhotos = [];
...
const DadosEmpreendimentoPage = ({ termo, empreendimento, unidade, t, formulario, pdfMode }) => (
  <div className="px-4 py-2 flex flex-col" style={{ minHeight: 'calc(297mm - 125px)' }}>
    <div>
      ...
      <div className="grid grid-cols-2 gap-x-8 gap-y-4 p-4 text-sm" style={{ border: '1px solid #e2e8f0', borderRadius: '0.375rem' }}>
        ...
        <div>
          ...
          <span>{termo?.revisao || t.notApplicable}</span>
        </div>
        <div className="col-span-2">
          <p className="font-semibold text-gray-700">Resp. Gerência:</p>
          <span>Engª Valéria Rodrigues</span>
        </div>
        {termo?.participantes && termo.participantes.trim() !== '' && (
          <div className="col-span-2">
            <p className="font-semibold text-gray-700">Participantes:</p>
            <div className="mt-1">
              {(() => {
                const participantesArray = termo.participantes.split(',').map(p => p.trim()).filter(Boolean);
                const rows = [];
      ...
      <div className="mt-6 space-y-4">
        {(termo?.texto_os_proposta && termo.texto_os_proposta.trim() !== '') && (
          <div>
            ...
            <p className="bg-gray-50 p-3 rounded-md border text-gray-700 whitespace-pre-wrap">
              {termo.texto_os_proposta}
            </p>
          </div>
        )}

        {(termo?.texto_escopo_consultoria && termo.texto_escopo_consultoria.trim() !== '') && (
          <div>
            ...
            <div className="bg-gray-50 p-3 rounded-md border text-gray-700">
              <div className="whitespace-pre-wrap">
                {termo.texto_escopo_consultoria}
              </div>
            </div>
          </div>
        )}
    ...
    <div className="flex-grow flex items-end justify-center pb-24">
      <div className="text-center text-sm">
        ...
        <p>{formulario?.consultor_responsavel || termo?.consultor_responsavel || t.notApplicable}</p>
      </div>
    </div>
  </div>
);
...
const ContentPage = ({ items, observacoes, sectionOriginalOrder, disciplineStats, sectionsToRenderObservationsOnThisPage, pdfMode, sectionNumberMap, layoutRelatorio = 'vertical', isTermoDeAceite = true }) => {
  return (
                            ...
                            <>
                              ...
                              <td className="border border-gray-300 p-2 align-middle text-center w-56">
                                {item.assinatura ? (
                                  ...
                                  item.resposta && item.resposta !== '-' &&
                                  <span className={`px-2 py-1 rounded text-xs font-medium inline-block ${getStatusBadgeClassFromColor(item.cor)}`}>
                                    {item.resposta}
                                  </span>
                                )}
                              </td>
...
const paginateContent = (allItems, observacoesSecoesProcessadas, termo, unidade, empreendimento, t, formulario, disciplineStats) => {
  const pages = [];
  ...
  if (empreendimento) {
    pages.push(
      <DadosEmpreendimentoPage termo={termo} empreendimento={empreendimento} unidade={unidade} t={t} formulario={formulario} />
    );
  }
  ...
  const hasAssinaturas = termo.assinaturas && termo.assinaturas.length > 0;
  if (hasAssinaturas) {
    const totalAssinaturas = termo.assinaturas.length;
  ...
  if (hasPhotos) {
    pages.push(<QRCodesPage allItems={allItems} termo={termo} />);
  }

  return pages;
};

const processReportData = (termo, formulario) => {
  if (!termo?.respostas || !formulario?.secoes) {
    return {
      allItems: [],
      dashboardData: [],
  ...
  let respostasRaw = termo.respostas;
  ...
  let fotosRaw = termo.fotos_secoes;
  ...
  let observacoesRaw = termo.observacoes_secoes;
...
const ReportContent = ({ termo, formulario, unidade, empreendimento, t, navigate }) => {
  const [isPrintingMode, setIsPrintingMode] = useState(false);
  const [user, setUser] = useState(null);
  const [loadingUser, setLoadingUser] = useState(true);

  const processedReportData = useMemo(() => {
    return processReportData(termo, formulario);
  }, [termo, formulario]);
  ...
  }, [processedReportData, termo, unidade, empreendimento, t, formulario]);
...
export default function VisualizarTermoAceite() {
    ...
    const loadTermoData = async () => {
      try {
        ...
        if (!isValidId(termoId)) {
          setError("ID do termo não fornecido na URL ou inválido.");
          setLoading(false);
          return;
        }

        const termoData = await TermoDeAceite.get(termoId);
        ...
        const [unidadeResult, empreendimentoResult] = await Promise.allSettled([
          ...
          termoData.id_empreendimento ? Empreendimento.get(termoData.id_empreendimento) : Promise.resolve(null),
        ]);
        ...
        if (termoData.id_formulario) {
          try {
            const fetchedForm = await FormularioVistoriaEntity.get(termoData.id_formulario);
        ...
        if (!loadedForm && termoData.estrutura_formulario && Array.isArray(termoData.estrutura_formulario) && termoData.estrutura_formulario.length > 0) {
          loadedForm = {
            nome_formulario: termoData.nome_termo || "Termo Avulso",
            secoes: termoData.estrutura_formulario,
            consultor_responsavel: termoData.consultor_responsavel
          };
        }
        ...
        if (loadedForm && loadedForm.secoes && Array.isArray(loadedForm.secoes) && loadedForm.secoes.length > 0) {
          ...
          setError(t.formNotFound(termoData?.id_formulario || 'N/A'));
          setFormulario(null);
        }
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-8">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md text-center">
          ...
          <p className="text-gray-700 mb-6">
            Alguns dados necessários para gerar o relatório não foram encontrados.
          </p>
          <Button
            onClick={() => navigate(-1)}
            variant="outline"
