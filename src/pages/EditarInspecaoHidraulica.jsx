...
import { InspecaoHidraulica } from '@/api/entities';
...
const t = {
  title: "Editar Inspeção Hidráulica",
...
export default function EditarInspecaoHidraulica() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const data = await InspecaoHidraulica.get(inspecaoId);
        let dataInspecao = '';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await InspecaoHidraulica.update(id, updateData);
      toast.success("Inspeção atualizada com sucesso!");
      navigate(createPageUrl(`EmpreendimentoInspecaoHidraulica?empreendimentoId=${formData.id_empreendimento}`));
    } catch (error) {
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowCoverDialog(true)}><Edit2 className="w-4 h-4 mr-2" />Editar Capa</Button>
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={formData.titulo_capa || 'RELATÓRIO'} onChange={e => handleInputChange('titulo_capa', e.target.value)} placeholder="Ex: RELATÓRIO" />
                  </div>
                  <div className="space-y-2">
                    <Label>Subtítulo da Capa (vermelho)</Label>
                    <Input value={formData.subtitulo_capa || 'Gerenciamento de Obra'} onChange={e => handleInputChange('subtitulo_capa', e.target.value)} placeholder="Ex: Gerenciamento de Obra" />
                  </div>
                </div>
              </div>
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg space-y-4">
                <h4 className="text-sm font-semibold text-blue-900">Informações Centrais da Capa</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label>Nome do Empreendimento (grande)</Label>
                    <Input value={formData.cliente} onChange={e => handleInputChange('cliente', e.target.value)} placeholder="Ex: MOST MOEMA" />
                  </div>
                  <div className="space-y-2">
                    <Label>Informações Adicionais (pequeno)</Label>
                    <Input value={formData.subtitulo_relatorio} onChange={e => handleInputChange('subtitulo_relatorio', e.target.value)} placeholder="Ex: Ed. Most Moema | MPD" />
                  </div>
                </div>
              </div>
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg space-y-2">
                <h4 className="text-sm font-semibold text-red-900">Rodapé da Capa (barra vermelha inferior)</h4>
                <Label>Texto do Rodapé</Label>
                <Input value={formData.texto_rodape_capa || ''} onChange={e => handleInputChange('texto_rodape_capa', e.target.value)} placeholder="Ex: Most Moema | Ed. Most Moema | MPD" />
                <p className="text-xs text-gray-500">Este texto será exibido na barra vermelha no rodapé da capa</p>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowCoverDialog(false)}>Fechar</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
