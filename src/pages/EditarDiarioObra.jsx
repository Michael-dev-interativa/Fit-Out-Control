...
export default function EditarDiarioObra() {
    ...
    useEffect(() => {
        if (!diarioId) {
            if(location.pathname.includes('EditarDiarioObra')) {
                 toast.error("ID do diário não encontrado na URL.");
                 navigate(-1);
            }
            return;
        }
        ...
        const loadData = async () => {
            setLoading(true);
            try {
                const [diarioData, unidadesData] = await Promise.all([
                    DiarioDeObra.get(diarioId),
                    empreendimentoId ? UnidadeEmpreendimento.filter({ id_empreendimento: empreendimentoId }) : Promise.resolve([])
                ]);
                
                const existingEfetivo = diarioData.efetivo || [];
                const combinedEfetivo = [...existingEfetivo];

                categoriasEfetivoPredefinidas.forEach(predefinedCat => {
                    if (!existingEfetivo.some(e => e.categoria === predefinedCat)) {
                        combinedEfetivo.push({ categoria: predefinedCat, presente: 0, ausente_com_justificativa: 0, ausente_sem_justificativa: 0 });
                    }
                });

                setFormData({
                    ...diarioData,
                    unidade_texto: diarioData.unidade_texto || '', // Initialize new field
                    nome_arquivo: diarioData.nome_arquivo || '', 
                    data_diario: diarioData.data_diario ? diarioData.data_diario.split('T')[0] : new Date().toISOString().split('T')[0],
                    id_unidade: diarioData.id_unidade || '',
                    condicao_climatica: diarioData.condicao_climatica || "Dias produtivos",
                    efetivo: combinedEfetivo,
                    principais_atividades: diarioData.principais_atividades?.length > 0 ? diarioData.principais_atividades : [''],
                    ocorrencias_observacoes: diarioData.ocorrencias_observacoes || '',
                    fotos: diarioData.fotos || [],
                    periodo_trabalhado: diarioData.periodo_trabalhado || { manha: true, tarde: true, noite: false },
                    horas_paralisadas: diarioData.horas_paralisadas || 0,
                    vistos: diarioData.vistos || []
                });
        ...
        loadData();
    }, [diarioId, empreendimentoId, navigate, location.pathname]);
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            const { id, created_date, updated_date, created_by, ...finalData} = formData;
            finalData.principais_atividades = finalData.principais_atividades.filter(a => a.trim() !== '');

            await DiarioDeObra.update(diarioId, finalData);
            toast.success("Diário de Obra atualizado com sucesso!");
            navigate(createPageUrl(`EmpreendimentoDiariosObra?empreendimentoId=${formData.id_empreendimento}`));
        } catch (error) {
            console.error("Erro ao atualizar diário:", error);
            toast.error("Falha ao atualizar o diário de obra.");
        } finally {
            setSaving(false);
        }
    };
                            ...
                            <tbody>
                                {formData.efetivo.map((item, index) => (
                                    <tr key={index} className="border-t">
                                        <td className="p-2"><Input type="text" value={item.categoria} onChange={(e) => handleEfetivoChange(index, 'categoria', e.target.value)} placeholder={t.categoryPlaceholder} /></td>
                                        ...
                                        <td className="text-center"><Button type="button" variant="ghost" size="icon" onClick={() => removeEfetivoCategoria(index)}><Trash2 className="w-4 h-4 text-red-500" /></Button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
