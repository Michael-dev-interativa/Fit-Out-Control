...
const SimpleSignaturePad = React.forwardRef((props, ref) => {
...
const t = {
    title: "Editar Inspeção de Alarme de Incêndio",
...
export default function EditarInspecaoAlarme() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                const data = await InspecaoAlarmeIncendio.get(inspecaoId);
                let dataInspecao = '';
                ...
                console.log('[LOAD] Assinaturas do banco:', data.assinaturas);
                setFormData({
                    ...data,
                ...
                console.log('[LOAD] Assinaturas processadas:', (data.assinaturas || []).map(a => ({ parte: a.parte, nome: a.nome, tem_img: !!a.assinatura_imagem })));
            } catch (error) {
    ...
    const handleAssinaturaChange = (index, field, value) => {
        console.log(`[CHANGE] Assinatura ${index} - campo ${field}:`, value?.substring(0, 80));
        const newAssinaturas = [...formData.assinaturas];
        newAssinaturas[index][field] = value;
        handleInputChange('assinaturas', newAssinaturas);
        console.log(`[CHANGE] Estado atualizado das assinaturas:`, newAssinaturas.map(a => ({ parte: a.parte, nome: a.nome, tem_img: !!a.assinatura_imagem })));
    };

    const addAssinatura = () => {
        console.log('[ADD] Adicionando nova assinatura');
        handleInputChange('assinaturas', [...formData.assinaturas, { parte: '', nome: '', assinatura_imagem: '' }]);
    };
    ...
    const handleSaveSignature = async () => {
        ...
        try {
            if (signatureMode === 'type') {
                ...
                const { file_url } = await UploadFile({ file });
                console.log('[UPLOAD] URL da assinatura digitada:', file_url);
                ...
                if (signaturePadRef.current && !signaturePadRef.current.isEmpty()) {
                    ...
                    const { file_url } = await UploadFile({ file });
                    console.log('[UPLOAD] URL da assinatura desenhada:', file_url);
                    handleAssinaturaChange(activeSignatureIndex, 'assinatura_imagem', file_url);
                    setShowSignatureDialog(false);
                    setActiveSignatureIndex(null);
                    toast.success("Assinatura salva! URL: " + file_url.substring(0, 50) + "...");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await InspecaoAlarmeIncendio.update(id, updateData);
            console.log('[SUBMIT] Inspeção atualizada com sucesso!');
            toast.success("Inspeção atualizada com sucesso!");
            navigate(createPageUrl(`EmpreendimentoInspecaoAlarme?empreendimentoId=${formData.id_empreendimento}`));
        } catch (error) {
                        ...
                        {formData.assinaturas.map((assinatura, index) => (
                            <div key={index} className="p-4 border rounded-lg bg-gray-50 space-y-3">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {console.log(`[RENDER] Assinatura ${index}:`, { parte: assinatura.parte, nome: assinatura.nome, tem_img: !!assinatura.assinatura_imagem, url: assinatura.assinatura_imagem?.substring(0, 80) })}
