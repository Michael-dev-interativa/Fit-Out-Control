        ...
        <div className={showHeader ? "p-4" : "px-4 pt-6 pb-4"}>
            {itensSlice && itensSlice.length > 0 && (
                <div>
                    {showHeader && (
                        <>
                            <h3 className="text-lg font-bold mb-2 bg-blue-900 text-white p-2 text-center">Inspeção Física - Gás</h3>
                        ...
                        <tbody>
                            ...
                            {showComments && (
                                <tr className="bg-gray-50">
                                    <td className="border border-black p-2 font-bold">Comentários:</td>
                                    <td className="border border-black p-2" colSpan="4" style={{ wordWrap: 'break-word', whiteSpace: 'pre-wrap', overflowWrap: 'break-word', wordBreak: 'break-word' }}>{local.comentarios || ''}</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
    ...
    return (
        <div className="report-page">
            {!isCover && (
                <div className="flex justify-between items-center border-b border-gray-200 bg-white" style={{ position: 'absolute', top: 0, left: 0, right: 0, height: HEADER_HEIGHT, zIndex: 100, padding: '4px 8px', maxWidth: '210mm', boxSizing: 'border-box' }}>
                    ...
                    <div className="text-right" style={{ flex: 1, paddingLeft: '8px', overflow: 'hidden' }}>
                        <h2 className="text-[10px] font-bold text-gray-800 uppercase leading-tight truncate">{relatorio?.titulo_relatorio || 'INSPEÇÃO DE GÁS'}</h2>
            ...
            <div className="border-t border-gray-200 bg-gray-50 flex justify-between items-center text-[9px] text-gray-500" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: FOOTER_HEIGHT, padding: '4px 8px', maxWidth: '210mm', boxSizing: 'border-box' }}>
                <div className="flex-1 text-left leading-tight truncate" style={{ paddingRight: '8px' }}><span className="font-medium">Arquivo: </span><span>{relatorio.nome_arquivo ? `${relatorio.nome_arquivo}.pdf` : `IGAS-${relatorio.id?.slice(-4)}.pdf`}</span></div>
...
const ReportContent = ({ relatorio, empreendimento, navigate }) => {
    ...
    const contentPages = relatorio.locais.flatMap((local) =>
        paginateLocalItemsForPrinting(local, { splitPhotoRows: true, photoChunkSize: 2, photoMaxHeightPx: 160 })
    );

    const assinaturasValidas = (relatorio.assinaturas || []).filter(ass =>
    ...
    const measureConclusaoHeight = () => {
        ...
        tempDiv.innerHTML = `
            <div style="padding:0 16px 16px 16px;">
                <h2 style="font-size:1.25rem;font-weight:700;text-align:center;margin:0 0 12px 0;background:#1e3a8a;color:white;padding:8px;">Conclusão</h2>
                <div style="border:1px solid #ccc;padding:10px 14px;display:flex;gap:40px;margin-bottom:12px;min-height:110px;"></div>
                <div style="border:1px solid #ccc;padding:12px;background:#f9fafb;min-height:56px;"></div>
            </div>
        `;
        document.body.appendChild(tempDiv);
        const h = tempDiv.offsetHeight;
    ...
    const measureAssinaturasHeight = () => {
        if (!hasAssinaturas) return 0;
        if (typeof document === 'undefined' || !document.body) {
            return 120 + (assinaturasValidas.length * 120);
        }
        const tempDiv = document.createElement('div');
        tempDiv.style.cssText = 'position:absolute;visibility:hidden;width:190mm;box-sizing:border-box;font-family:Inter,Poppins,sans-serif;font-size:10px;left:-9999px;';
        tempDiv.innerHTML = `
            <div style="padding:16px;">
                <h2 style="font-size:1.25rem;font-weight:700;text-align:center;margin:0 0 16px 0;background:#1e3a8a;color:white;padding:8px;">Assinaturas</h2>
                <div style="margin-top:32px;display:flex;flex-direction:column;gap:24px;">
                    ${assinaturasValidas.map(() => `
                        <div style="display:flex;flex-direction:column;align-items:center;">
                            <div style="margin-bottom:8px;border-bottom:2px solid #000;width:256px;height:96px;"></div>
                            <div style="text-align:center;height:32px;"></div>
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    const assinaturasHeightPx = measureAssinaturasHeight();
    ...
    const remainingAfterInlineConclusaoPx = inlineConclusao ? (remainingAfterInlineObsPx - conclusaoHeightPx) : 0;
    const inlineAssinaturas = hasAssinaturas && inlineConclusao && remainingAfterInlineConclusaoPx >= assinaturasHeightPx;
    ...
    if (hasAssinaturas && !inlineAssinaturas) tailBlocks.push({ type: 'assinaturas', estimatedHeightPx: assinaturasHeightPx });
    ...
    const totalPages = 1 + (hasDocumentacao ? 1 : 0) + contentPages.length + tailPages.length + 1;
    let currentPage = 1;
        ...
        <div className="bg-gray-200 print:bg-white min-h-screen font-sans">
            <div className="no-print shadow-sm border-b p-4 mb-4 bg-white">
                <div className="flex justify-between items-center max-w-4xl mx-auto">
                    ...
                    <h1 className="text-xl font-semibold text-gray-800">Visualizar Inspeção de Gás</h1>
            ...
            <div className="report-container max-w-4xl mx-auto" style={{ padding: 0 }}>
                ...
                {contentPages.map((page, index) => (
                    ...
                    >
                        ...
                        {inlineAssinaturas && index === contentPages.length - 1 && (
                            <AssinaturasPage assinaturas={assinaturasValidas} />
                        )}
                    </ReportPageLayout>
                ))}
                    ...
                    >
                        {tailPageBlocks.map((block, blockIndex) => {
                            ...
                            if (block.type === 'conclusao') {
                                return <ConclusaoPage key={`tail-conc-${blockIndex}`} conclusaoR01={relatorio.conclusao_r01} conclusaoR02={relatorio.conclusao_r02} />;
                            }
                            if (block.type === 'assinaturas') {
                                return (
                                    <AssinaturasPage
                                        key={`tail-sign-${blockIndex}`}
...
export default function VisualizarInspecaoGas() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const relatorioData = await base44.entities.InspecaoGas.get(relatorioId);
