...
import { NaoConformidade } from '@/api/entities';
...
const t = {
    ...
    engObra: "Eng. Obra",
    revisao: "Revisão",
    nomeArquivo: "Nome do Arquivo (sem .pdf)",
...
const cronogramaOptions = ["OK", "Parcial", "Pendente"];

export default function EditarNaoConformidade() {
    const navigate = useNavigate();
    const location = useLocation();
    const relatorioId = new URLSearchParams(location.search).get('id');
    ...
    const handleOpenEditCover = () => {
        setEditedFormData({
            ...
            titulo_relatorio: formData.titulo_relatorio,
            subtitulo_relatorio: formData.subtitulo_relatorio,
            cliente: formData.cliente,
            eng_obra: formData.eng_obra,
    ...
    const handleSaveCover = async () => {
        try {
            const updatedData = {
                ...
                titulo_relatorio: editedFormData.titulo_relatorio,
                subtitulo_relatorio: editedFormData.subtitulo_relatorio,
                cliente: editedFormData.cliente,
                eng_obra: editedFormData.eng_obra,
                revisao: editedFormData.revisao,
                nome_arquivo: editedFormData.nome_arquivo
            };
            setFormData(updatedData);
            setEditCoverOpen(false);
            toast.success("Campos da capa atualizados!");
        } catch (err) {
            toast.error("Erro ao atualizar campos da capa.");
        }
    };
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (relatorioId) {
                try {
                    const data = await NaoConformidade.get(relatorioId);
                    setFormData(data);
                } catch (error) {
    ...
    if (!formData) return <div className="p-6">Dados não encontrados</div>;
    ...
    const addSection = () => {
        setFormData(p => ({ ...p, secoes: [...p.secoes, { nome_secao: '', itens: [{ local: '', complemento: '', disciplina: '', anomalia: '', plano_melhoria: '', cronograma_atividade: 'Pendente', data_status: '', responsavel: '', prazo: '', fotos: [] }] }] }));
    };
    ...
    const addItem = (secIndex) => {
        const newSecoes = [...formData.secoes];
        newSecoes[secIndex].itens.push({ local: '', complemento: '', disciplina: '', anomalia: '', plano_melhoria: '', cronograma_atividade: 'Pendente', data_status: '', responsavel: '', prazo: '', fotos: [] });
        setFormData(p => ({ ...p, secoes: newSecoes }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await NaoConformidade.update(relatorioId, dataToSubmit);
                        ...
                        <div className="border rounded-lg p-4 bg-blue-50">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input value={editedFormData?.titulo_capa || 'RELATÓRIO'} onChange={(e) => setEditedFormData({...editedFormData, titulo_capa: e.target.value})} className="mt-1" />
                                </div>
                                <div>
                                    <Label className="text-xs font-semibold">Subtítulo da Capa (vermelho)</Label>
                                    <Input value={editedFormData?.subtitulo_capa || 'Gerenciamento de Obra'} onChange={(e) => setEditedFormData({...editedFormData, subtitulo_capa: e.target.value})} className="mt-1" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="border rounded-lg p-4 bg-blue-50">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <Input value={editedFormData?.titulo_relatorio || ''} onChange={(e) => setEditedFormData({...editedFormData, titulo_relatorio: e.target.value})} className="mt-1" />
                                </div>
                                <div>
                                    <Label className="text-xs font-semibold">Informações Adicionais (pequeno)</Label>
                                    <Input value={editedFormData?.subtitulo_relatorio || ''} onChange={(e) => setEditedFormData({...editedFormData, subtitulo_relatorio: e.target.value})} placeholder="Ex: 1º subsolo: Central de água quente Estúdios" className="mt-1" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="border rounded-lg p-4 bg-gray-50">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input value={editedFormData?.cliente || ''} onChange={(e) => setEditedFormData({...editedFormData, cliente: e.target.value})} className="mt-1" />
                                </div>
                                <div>
                                    <Label className="text-xs font-semibold">Engenheiro da Obra</Label>
                                    <Input value={editedFormData?.eng_obra || ''} onChange={(e) => setEditedFormData({...editedFormData, eng_obra: e.target.value})} className="mt-1" />
                                </div>
                                <div className="col-span-2">
                                    <Label className="text-xs font-semibold">Revisão</Label>
                                    <Input value={editedFormData?.revisao || ''} onChange={(e) => setEditedFormData({...editedFormData, revisao: e.target.value})} className="mt-1" />
                                </div>
                                <div>
                                    <Label className="text-xs font-semibold">Nome do Arquivo (sem .pdf)</Label>
                                    <Input value={editedFormData?.nome_arquivo || ''} onChange={(e) => setEditedFormData({...editedFormData, nome_arquivo: e.target.value})} className="mt-1" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <Card>
                    ...
                    <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-2"><Label>{t.date}</Label><Input type="date" value={formData.data_vistoria?.split('T')[0]} onChange={e => handleInputChange('data_vistoria', e.target.value)} required /></div>
                        ...
                        <div className="space-y-2"><Label>{t.nomeArquivo}</Label><Input value={formData.nome_arquivo || ''} onChange={e => handleInputChange('nome_arquivo', e.target.value)} /></div>
                    </CardContent>
                </Card>
                                ...
                                {secao.itens.map((item, itemIndex) => (
                                    <div key={itemIndex} className="border-t pt-4 mt-4 space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div className="space-y-2">
                                                <Label>Prazo</Label>
                                                <Input type="date" value={item.prazo || ''} onChange={e => handleItemChange(secIndex, itemIndex, 'prazo', e.target.value)} />
                                            </div>
                                        </div>
                                    </div>
                                ))}
