...
import { Empreendimento } from '@/api/entities'; import { getUploadUrl } from '@/api/config'; import { compressReportImages } from '@/lib/compressReportImages'; import { Button } from '@/components/ui/button';
    ...
    return (
        <>
            ...
            <div className="absolute z-30" style={{ top: '10%', right: '8%', width: '50%', textAlign: 'right' }}>
                ...
                <h2 style={{ fontFamily: "'Inter', sans-serif", fontSize: '22px', color: redColor, letterSpacing: '1px' }}>INSPEÇÃO DE ALARME DE INCÊNDIO</h2>
            </div>
    ...
    return (
        <div className={combineWithDoc ? "px-4 pb-4" : (isFirstPageOfLocal ? "p-4" : "px-4 pt-6 pb-4")}>
            {items && items.length > 0 && (
                <div>
                    <h3 className="text-lg font-bold mb-2 bg-blue-900 text-white p-2 text-center">Inspeção Física dos Equipamentos de SDAI</h3>
                    <div className="mb-4 border border-black">
                        <div className="p-2">
                            <span className="font-bold">DATA CENTER/CPD - LOCAL: </span>{local.nome_local}
                        </div>
                    </div>
                                ...
                                return (
                                    <>
                                        <tr key={idx} style={{ pageBreakInside: 'avoid', pageBreakAfter: 'auto' }}>
                                            ...
                                            <td className="border border-black p-2" style={{ width: '44%', wordWrap: 'break-word', wordBreak: 'break-word', whiteSpace: 'pre-wrap' }}>{item.observacoes || ''}</td>
                                        </tr>
            ...
            {!isCover && (
                <div className="flex justify-between items-center border-b border-gray-200 bg-white" style={{ position: 'absolute', top: 0, left: 0, right: 0, height: HEADER_HEIGHT, zIndex: 100, padding: '4px 8px', maxWidth: '210mm', boxSizing: 'border-box' }}>
                    ...
                    <div className="text-right" style={{ flex: 1, paddingLeft: '8px', overflow: 'hidden' }}>
                        <h2 className="text-[10px] font-bold text-gray-800 uppercase leading-tight truncate">{relatorio?.titulo_relatorio || 'INSPEÇÃO DE ALARME DE INCÊNDIO'}</h2>
...
const ReportContent = ({ relatorio, empreendimento, navigate }) => {
    ...
    if (relatorio.assinaturas && relatorio.assinaturas.length > 0) {
        relatorio.assinaturas.forEach((ass, idx) => {
            console.log(`[VISUALIZAR] Assinatura ${idx}:`, {
                parte: ass.parte,
                nome: ass.nome,
                assinatura_imagem_exists: !!ass.assinatura_imagem,
                assinatura_imagem_type: typeof ass.assinatura_imagem,
                assinatura_imagem_length: ass.assinatura_imagem?.length,
                assinatura_imagem_value: ass.assinatura_imagem,
                is_trim_empty: ass.assinatura_imagem?.trim() === '',
                trim_result: ass.assinatura_imagem?.trim()
            });
        });
    }
    ...
    const totalPages = 1 + (hasDocumentacao && !combineDocWithContent ? 1 : 0) + contentPages.length + 2 + (hasAssinaturas ? 1 : 0);
        ...
        <div className="bg-gray-200 print:bg-white min-h-screen font-sans">
            <div className="no-print shadow-sm border-b p-4 mb-4 bg-white">
                <div className="flex justify-between items-center max-w-4xl mx-auto">
                    ...
                    <h1 className="text-xl font-semibold text-gray-800">Visualizar Inspeção de Alarme</h1>
...
export default function VisualizarInspecaoAlarme() {
