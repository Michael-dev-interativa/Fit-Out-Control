...
export default function EmpreendimentoInspecaoCFTV({ language: initialLanguage, theme: initialTheme }) {
    ...
    const loadData = async () => {
        ...
        try {
            const [empreendimentoData, inspecoesData] = await Promise.all([
                ...
                InspecaoCFTV.filter({ id_empreendimento: empreendimentoId }, "-data_inspecao")
            ]);
        ...
        <div className={`p-6 ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-50'}`}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={() => navigate(createPageUrl(`NovaInspecaoCFTV?empreendimentoId=${empreendimentoId}`))}>
                        ...
                        <Card key={inspecao.id} className={isDark ? 'bg-gray-800' : ''}>
                            ...
                            <CardFooter className="flex justify-end gap-2">
                                <div className="flex items-center gap-2">
                                    <Link to={createPageUrl(`VisualizarInspecaoCFTV?relatorioId=${inspecao.id}`)}>
                                        <Button variant="ghost" size="icon"><Eye className="w-4 h-4" /></Button>
                                    </Link>
                                    <Link to={createPageUrl(`EditarInspecaoCFTV?inspecaoId=${inspecao.id}`)}>
