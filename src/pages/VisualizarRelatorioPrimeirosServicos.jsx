...
const ReportPage = ({ children, pageNumber, totalPages, relatorio, empreendimento, pdfMode }) => {
  ...
  return (
    <div className={`report-page w-full relative bg-white ${pdfMode ? 'pdf-mode' : ''}`}>
      {/* Header - always rendered for content pages */}
      <div className="flex justify-between items-center p-4 border-b border-gray-200" style={{ position: 'absolute', top: 0, left: 0, right: 0, height: HEADER_HEIGHT }}>
          ...
          <div className="text-right">
            <h2 className="text-sm font-bold text-gray-800 uppercase">RELATÓRIO DE 1º SERVIÇOS</h2>
      ...
      <div className="overflow-hidden print:overflow-visible" style={{ paddingTop: HEADER_HEIGHT, paddingBottom: FOOTER_HEIGHT }}>
        {children}
      </div>
      {/* Footer - always rendered for content pages */}
      <div className="px-3 py-1 border-t border-gray-200 bg-gray-50 flex justify-between items-center text-xs text-gray-500" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: FOOTER_HEIGHT }}>
        <div className="flex-1 text-left"><span className="font-medium">Arquivo:</span><br /><span>{relatorio?.nome_arquivo || `RPS-${relatorio.id?.slice(-4)}.pdf`}</span></div>
...
const CoverPage = ({ relatorio, empreendimento, className }) => { // Added className prop
    ...
    return (
        <div className={`relative w-full h-full bg-white font-sans ${className || ''}`} style={{ margin: 0, padding: 5 }}> {/* Applied className here */}
            ...
            <div className="absolute z-30" style={{ top: '10%', right: '8%', width: '50%', textAlign: 'right' }}>
                ...
                <h2 style={{ fontFamily: "'Inter', sans-serif", fontSize: '29.5px', color: redColor, letterSpacing: '4px' }}>1º SERVIÇOS</h2>
            </div>
...
const ServicosContentPage = ({ relatorio, photos }) => {
    return (
        <div className="p-8 text-sm bg-white h-full">
            <div className="border-t border-l grid grid-cols-4">
                ...
                <div className="border-b border-r p-2 col-span-2"><p className="text-xs font-bold text-gray-600">Assunto do Relatório</p><p>{relatorio?.assunto_relatorio}</p></div>
            ...
            <div className="border-l border-r border-b p-2 min-h-[100px]"><p className="text-xs font-bold text-gray-600">Descrição dos Serviços</p><p>{relatorio?.descricao_relatorio}</p></div>
        ...
        <div className="p-8 text-sm bg-white break-inside-avoid">
            ...
            <div className="mt-8 border-t pt-4 flex-grow"><h2 className="text-center font-bold mb-2">APROVAÇÕES</h2>
                <div className="grid grid-cols-3 gap-[-1px]">
                    {(relatorio?.aprovacoes || []).slice(0, 3).map((aprov, index) => (
                    ...
                    {[...Array(3 - (relatorio?.aprovacoes?.length || 0))].map((_, i) => (
...
const paginateRelatorioContent = (relatorio, empreendimento) => {
    const pages = [];
    // A capa é um item especial agora, não precisa do wrapper ReportPage
    pages.push({ type: 'cover', content: <CoverPage relatorio={relatorio} empreendimento={empreendimento} /> });
    
    const allPhotos = relatorio?.fotos || [];
    const photosOnFirstPageCount = 4;
    const photosOnSubsequentPagesCount = 4;
    ...
    pages.push({ type: 'content', content: <ServicosContentPage relatorio={relatorio} photos={firstPagePhotos} /> });
    
    if (remainingPhotos.length === 0) {
        pages.push({ type: 'content', content: <AprovacaoPage relatorio={relatorio} /> });
        return pages;
    }
    ...
    for (const chunk of photoChunks) {
        pages.push({ type: 'content', content: <PhotoPage photos={chunk} /> });
    }
    ...
    pages.push({ type: 'content', content: <FinalPageContent photos={lastPhotoChunk} relatorioData={relatorio} /> });
...
const ReportContent = ({ relatorio, empreendimento, navigate }) => {
    ...
    const paginatedPages = useMemo(() => {
        return paginateRelatorioContent(relatorio, empreendimento);
    }, [relatorio, empreendimento]);

    const handlePrint = async () => {
        const originalTitle = document.title;
        document.title = `RPS-${empreendimento?.nome_empreendimento || 'Relatorio'}-${relatorio.id?.slice(-4) || '0000'}`;
        ...
        <div className="bg-gray-200 print:bg-white min-h-screen font-sans">
            <div className="no-print shadow-sm border-b p-4 mb-4 bg-white">
                <div className="flex justify-between items-center max-w-4xl mx-auto">
                    ...
                    <h1 className="text-xl font-semibold text-gray-800">Relatório de 1º Serviços</h1>
            ...
            <div className="report-container max-w-4xl mx-auto" style={{ padding: 0 }}>
                {paginatedPages.map((page, index) => {
                    if (page.type === 'cover') {
                        return (
                            <div key={`page-${index}`} className="report-page">
                                {React.cloneElement(page.content, { pdfMode: isPrintingMode, className: 'cover-page-content' })} {/* Added className prop */}
                            </div>
                        );
                    }
                    return (
            ...
            <style>{`
                ...
                @media print {
                  ...
                  .report-page { 
                    ...
                    width: 210mm; 
                    height: 297mm; /* Mantém a altura fixa para a visualização e como base */
                    margin: 0; 
                    padding: 0; 
                    page-break-inside: avoid; 
                    overflow: hidden; /* Mantém hidden como padrão para a capa */
                    box-shadow: none; 
                    position: relative; 
                  }
                  /* Aplica overflow visível para páginas de conteúdo, mas não para o elemento raiz da capa */
                  .report-page > div:not(.cover-page-content) { 
...
export default function VisualizarRelatorioPrimeirosServicos() {
